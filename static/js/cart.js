...
async function apiRemove(id){
  const r = await fetch(`/api/cart/remove/${id}`, { method:'DELETE' });
  return r.json();
}
async function apiClear(){
  const r = await fetch('/api/cart/clear', { method:'DELETE' });
  return r.json();
}

function updateBadge(count){
  if(!cartCount) return;
  cartCount.textContent = String(count || 0);
}
...
function renderCart(data){
  ...
  <div class="cart-item" data-id="${it.id}">
    ...
    <div style="display:flex; flex-direction:column; align-items:end; gap:.4rem;">
      ...
      <button class="btn danger remove" style="padding:.35rem .6rem; font-size:.85rem;">Remove</button>
    </div>
  </div>
`).join('');
...
checkoutBtn?.addEventListener('click', async ()=>{
  const data = await apiGetCart();
  alert('Proceeding to checkout. Subtotal: ' + fmtINR(data.subtotal));
});
